...
def reverse_recursive(lst):
    ...
    if len(lst) == 1:
        return lst
    return [lst[-1]] + reverse_recursive(lst[:len(lst)-1])
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    distance_a = distance(city_a, make_city('pos',lat,lon))
    distance_b = distance(city_b, make_city('pos',lat,lon))
...
def add_chars(w1, w2):
    ...
    ls = list(w1)
    def helper(s ,ls ):
        if type(s) == str and (s not in ls) and len(s) == 1:
            return s
        elif s in ls :
            del ls[0]
            return ''
        return helper(s[0],ls) + helper(s[1:],ls)
    return helper(w2,ls)
